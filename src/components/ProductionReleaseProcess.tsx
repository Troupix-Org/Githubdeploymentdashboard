...
export function ProductionReleaseProcess({
  ...
  const updateSteps = () => {
    ...
    const stagingDeployments = releaseDeployments.filter(
      (d) =>
    ...
    const stagingDeploymentsComplete = stagingDeployments.every(
      (d) => d.status === "success",
    );
    ...
    const prodDeploymentsComplete = prodDeployments.every(
      (d) => d.status === "success",
    );
...
${project.pipelines
  .filter(
    (p) =>
      p.environment?.toLowerCase().includes("staging") ||
      p.environment?.toLowerCase().includes("qa"),
  )
  .map((p) => `  • ${p.name} (${p.environment || p.branch})`)
  .join("\n")}
...
${project.pipelines
  .filter((p) => p.environment?.toLowerCase().includes("prod"))
  .map((p) => `  • ${p.name}`)
  .join("\n")}
...
Thanks and regards`,
        ...
        if (step.stepId === 2) {
          return {
            ...
            metadata: {
              ...step.metadata,
              emailSent: true,
              recipients: stagingEmailRecipients,
            },
          };
        }
        return step;
  ...
  const downloadComplianceFile = () => {
    ...
    const blob = new Blob([complianceFile.fileContent], {
      type: "text/plain",
    });
        ...
        if (step.stepId === 4) {
          return {
            ...
            metadata: {
              ...step.metadata,
              emailSent: true,
              recipients: prodEmailRecipients,
            },
          };
        }
        return step;
  ...
  const resetProcess = () => {
    if (
      confirm(
        "Are you sure you want to reset the entire production release process? This will clear all sign-offs and email records.",
      )
    ) {
  ...
  const markAllComplete = () => {
    if (
      confirm(
        "Are you sure you want to mark all steps as complete? This will bypass the entire production release process.",
      )
    ) {
      if (currentRelease) {
                      ...
                      {step.status === "completed" && (
                        <Button
                          ...
                          onClick={() => {
                            if (
                              confirm(
                                `Are you sure you want to reset "${step.title}"?`,
                              )
                            ) {
                              if (currentRelease) {
                                updateReleaseStep(
                                ...
                                localStorage.removeItem(
                                  `${STORAGE_PREFIX}step_${step.id}_completed`,
                                );
      ...
      >
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {selectedRepository && (
              <ReleaseCreator
                repository={project.repositories.find(
