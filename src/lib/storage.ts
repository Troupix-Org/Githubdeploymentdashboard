...
export interface ProductionRelease {
  ...
  }; // Deployment details by pipeline
  ...
  try {
    ...
    newProject.pipelines = data.project.pipelines.map((pipeline: any) => {
      ...
      return {
        ...
        id: newPipelineId,
        repositoryId:
          oldToNewRepoIds.get(pipeline.repositoryId) ||
          newProject.repositories[0]?.id ||
          "",
      };
    });
      ...
      if (Array.isArray(data.deployments)) {
        ...
        data.deployments.forEach((deployment: Deployment) => {
          ...
          const newDeployment: Deployment = {
            ...
            pipelineId:
              oldToNewPipelineIds.get(deployment.pipelineId) ||
              deployment.pipelineId,
            repositoryId:
              oldToNewRepoIds.get(deployment.repositoryId) ||
        ...
        if (
          newProject.isProductionRelease &&
          Array.isArray(data.productionReleases)
        ) {
          data.productionReleases.forEach((release: ProductionRelease) => {
            const newRelease: ProductionRelease = {
              ...
              projectId: newProjectId,
              deploymentIds: release.deploymentIds.map(
                (oldId) => oldToNewDeploymentIds.get(oldId) || oldId,
              ),
              stagingDeploymentIds: release.stagingDeploymentIds?.map(
                (oldId) => oldToNewDeploymentIds.get(oldId) || oldId,
              ),
              productionDeploymentIds: release.productionDeploymentIds?.map(
                (oldId) => oldToNewDeploymentIds.get(oldId) || oldId,
              ),
            };
...
export function getProductionReleaseById(
  id: string,
): ProductionRelease | undefined {
  return getProductionReleases().find((r) => r.id === id);
}

export function getProductionReleasesByProject(
  projectId: string,
): ProductionRelease[] {
